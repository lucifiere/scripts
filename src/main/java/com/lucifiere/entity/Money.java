...
public class Money implements Comparable, Serializable {
    ...
    private static final long serialVersionUID = 8175505740403764123L;
     ...
     */
    private static final int[] CENT_FACTOR = new int[]{1, 10, 100, 1000};
     ...
     */
    protected static Map<String, Map<String, String>> CURRENCY_DISPLAY_UNIT_MAP = new HashMap<>(2);

    //进行初始化工作
    static {
        //简体中文的对应表
        Map<String, String> zhCNMap = new HashMap<>(2);
        zhCNMap.put("CNY", "元");
        zhCNMap.put("POINT", "积分");
        CURRENCY_DISPLAY_UNIT_MAP.put("zh_CN", zhCNMap);

        //繁体中文的对应表
        Map<String, String> zhHKMap = new HashMap<>(2);
        zhHKMap.put("CNY", "元");
    ...
    public Money(long yuan, int cent) {
        this(yuan, cent, DEFAULT_CURRENCY);
    }
    ...
    public int getCentFactor() {
        return CENT_FACTOR[this.currency.getDefaultFractionDigits()];
    }
     ...
     *
     * @param other 另一对象。
     * @return true表示小于，false表示不小于（大于等于）。
     * @throws IllegalArgumentException 待比较货币对象与本货币对象的币种不同。
     */
     ...
     * @return 相加后的结果
     */
    public Money addForm(long cent) {
        this.cent += cent;
        return this;
    }
     ...
     * @throws IllegalArgumentException 如果本货币对象与另一货币对象币种不同。
     */
    public Money subtractFrom(long cent) {
    ...
    public <T> Money multiply(T val) {
        if (val instanceof Double) {
            return multiply((((Double) val).doubleValue()));
        } else if (val instanceof Long) {
            return multiply(((Long) val).longValue());
        }
        return Money.ZERO;
    }
    ...
    public String dump() {
        ...
        return "cent = " + this.cent + lineSeparator
    ...
    public String getDisplayUnit() {
        return getDisplayUnit(Locale.getDefault());
    }
    ...
    public String getDisplayUnit(Locale e) {
        ...
        Map cMap = CURRENCY_DISPLAY_UNIT_MAP.get(e.toString());
        if (cMap == null) {
            cMap = CURRENCY_DISPLAY_UNIT_MAP.get(DEFAULT_LOCALE);
        }
